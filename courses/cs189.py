...
class CS189(BaseCourse):
    ...
    def extract_data(self, data_json):
        try:
            ...
            message_enrollment = f"{enrolled} out of {max_enrolled} spots are taken."
            message_waitlist = f"{waitlisted} out of {max_waitlist} waitlist spots are taken."
            print(message_waitlist)
            return available, message_enrollment
